(ns tobaccoretailops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (TobaccoRetailAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the TobaccoRetailGovernor (:govern)
  ...
  human tobacco retail operations coordinator. The approver resumes with
...
(defn build
  ...
  `tobaccoretailops.store/Store`).
  opts:
    :advisor      -- a `tobaccoretailops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; TobaccoRetailAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; TobaccoRetailGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
