(ns tobaccoretailops.governor-contract-test
  ...
  (testing "clean sales-record proposal -> auto-commit at phase 3"
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t1"
                               {:op :log-sales-record :store-id "store-1" :patch {:units-sold 18 :item "cigars"}}
                               ctx)]
      (is (some? result))
    ...
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2"
                               {:op :flag-compliance-concern :store-id "store-1"
                                :patch {:concern "suspected underage sale" :confidence 0.99}}
                               ctx)]
      (is (some? result))
...
(deftest unlicensed-store-hard-hold
  (testing "registered but unlicensed store -> permanent HARD hold"
    (let [db (store/seed-db)
          actor (op/build db)
      ...
      (is (some? result))
      (is (= 0 (count (store/coordination-log db)))
          "unlicensed store must HARD hold"))))
...
(deftest scope-excluded-content-hard-hold
  (testing "proposal drifting into age-verification-override scope -> permanent hard hold"
    (let [db (store/seed-db)
          actor (op/build db)
